...
VAE uses a loss function that is a sum of two components: 1) **MSE reconstruction error** between input and output, similar to AE. It kinda makes sense, because we want decoder output to be close to original input. 2) **Regularization term** enforcing that all Gaussians keep close to the standard normal $\mathcal{N}(0, 1)$ during the training. Mathematically, this term represents a [KL Divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence) between our Gaussian and standard normal.
